...
class ChatRoom(SoftDeleteModel):
    ...
    def _get_forum_name(self):
        ...
        if len(metas) > 0:
            ...
            return self.puzzle.hunt.settings.discord_unassigned_text_category

    def create_channels(self):
        service = self.get_service()
        update_fields = []
        if self.text_channel_id is None:
            forum_name = self._get_forum_name()
            forum_channel_id = service._get_or_create_forum_channel(
                self.get_guild_id(), forum_name
            )

            self.text_channel_id, self.text_channel_url = service.create_forum_post(
                forum_channel_id,
                self.puzzle.name,
                f"**{self.puzzle.name}** has been unlocked!",
                [PuzzleTag.NEW_TAG_ID],
                self.puzzle,
            )
            update_fields.extend(["text_channel_id", "text_channel_url"])

        # Audio channels are no longer used, but we keep the fields to avoid migrations.

        self.save(update_fields=update_fields)

    # The following methods are no longer needed for forum post management,
    # but we'll keep them to avoid migrations.
    ...
    def handle_tag_added(self, puzzle, tag_name):
        # Only handle tags that should trigger an announcement in the announcement channel.
    ...
    def handle_tag_removed(self, puzzle, tag_name):
        # We don't need to do anything specific when a tag is removed from the post itself.
    ...
    def handle_puzzle_rename(self, new_name):
        service = self.get_service()
        if self.text_channel_id:
            service.edit_forum_post_name(self.text_channel_id, new_name)
...
def delete_chat_room_channels(sender, instance, using, **kwargs):
    # No need to delete anything here now
    pass